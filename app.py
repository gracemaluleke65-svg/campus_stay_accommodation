...
@app.route('/admin/nuke-all', methods=['GET', 'POST'])
@login_required
def nuke_all():
    if not current_user.is_admin:
        flash('Access denied.', 'danger')
    ...
    if request.method == 'POST':
        try:
            ...
            count_fav = Favorite.query.delete()
            count_rev = Review.query.delete()
            count_book = Booking.query.delete()
            count_acc = Accommodation.query.delete()
            db.session.commit()
            ...
            <html>
            <head>
                <title>Data Wiped</title>
                <style>
                    body {{ font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px; }}
                    .success {{ background: #d4edda; color: #155724; padding: 20px; border-radius: 10px; }}
                    a {{ display: inline-block; margin-top: 20px; padding: 10px 20px; background: #ff6b35; color: white; text-decoration: none; border-radius: 5px; }}
                </style>
            </head>
            <body>
                <div class="success">
                    <h1>✅ All Data Wiped Successfully!</h1>
                    <p>Deleted:</p>
                    <ul>
                        <li>{count_fav} favorites</li>
                        <li>{count_rev} reviews</li>
                        <li>{count_book} bookings</li>
                        <li>{count_acc} accommodations</li>
                    </ul>
                </div>
    ...
    <html>
    <head>
        <title>⚠️ NUCLEAR OPTION</title>
        <style>
            body { font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px; text-align: center; }
            .warning { background: #fff3cd; border: 3px solid #ff6b35; padding: 30px; border-radius: 15px; }
            h1 { color: #dc3545; margin-top: 0; }
            .btn-delete { background: #dc3545; color: white; border: none; padding: 20px 40px; font-size: 18px; border-radius: 10px; cursor: pointer; margin-top: 20px; }
            .btn-delete:hover { background: #c82333; }
            .btn-cancel { display: inline-block; margin-top: 20px; padding: 15px 30px; background: #6c757d; color: white; text-decoration: none; border-radius: 10px; }
        </style>
    </head>
    <body>
        <div class="warning">
            ...
            <form method="POST">
                <button type="submit" class="btn-delete" onclick="return confirm('Are you absolutely sure?')">
